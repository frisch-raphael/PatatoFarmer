...
class SetActionParamUsage():
    ...
    def no_param():
        ...
        SetActionParamUsage.__base_help()
...
class SetAction(BaseActionWithTarget):
    ...
    def __test_usage_correctness_and_display_help(self, command_parts: list[str]):
        ...
        if len(command_parts) >= 1:
            ...
            if not hasattr(self.target, param_name):
                SetActionParamUsage.invalid_param(param_name)
                return False
    ...
    def execute(self, command_parts):
        ...
        if len(values) == 1:
            setattr(self.target, parameter_name, values[0])
        else:
            setattr(self.target, parameter_name, values)
